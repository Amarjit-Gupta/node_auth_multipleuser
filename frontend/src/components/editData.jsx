...
const EditData = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        else if (String(inputValue.contact).length !== 10) {
            toast.warn("please enter 10 digit in contact no...");
            return;
        }
        else if (String(inputValue.rollNo).trim() && inputValue.name.trim() && inputValue.email.trim() && String(inputValue.contact).trim() && inputValue.course.trim()) {
            setLoading(true);
            try {
                let result = await fetch(`${URL}/data/update/${index}`, {
                    method: "put",
    ...
    const getSingleData = async () => {
        let result = await fetch(`${URL}/data/getSingle/${index}`, {
            headers: {
                "authorization": `bearer ${JSON.parse(localStorage.getItem("userToken"))}`
            }
        });
        let data = await result.json();
        if (data.success) {
            let rollNo = data?.result.rollNo;
            let name = data?.result.name;
            let email = data?.result.email;
            let contact = data?.result.contact;
            let course = data?.result.course;
            setInputValue({
                rollNo: rollNo,
                name: name,
                email: email,
    ...
    return (
        <div>
            ...
            <div className="border bg-gray-100 w-80 pt-6 pl-8 pr-8 pb-8 m-auto mt-20 rounded-xl md:w-100">
                <p className="text-center text-3xl">EditData</p>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="border mt-5 h-10 rounded-2xl flex justify-center items-center gap-2 text-xl hover:bg-gray-200">
                        {/* <button type="submit" className="cursor-pointer h-[100%] w-[100%] border-0 rounded-xl">Update</button> */}
                        <button type="submit" disabled={loading} className="cursor-pointer h-[100%] w-[100%] border-0 rounded-xl disabled:text-gray-400">{loading ? <span className="flex justify-center items-center gap-3">Updating... <LuLoaderCircle className="animate-spin" /></span> : "Update"}</button>
                    </div>
