...
const Login = () => {
    ...
    return (
        <div>
            <Navbar />
            <div className="border bg-gray-100 w-80 pt-6 pl-8 pr-8 pb-8 m-auto mt-30 rounded-xl md:w-100">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="border mt-5 h-10 rounded-2xl flex justify-center items-center gap-2 text-xl hover:bg-gray-200">
                        {/* <button type="submit" className="cursor-pointer h-[100%] w-[100%] border-0 rounded-xl">Login</button> */}
                        <button type="submit" disabled={loading} className="cursor-pointer h-[100%] w-[100%] border-0 rounded-xl disabled:text-gray-400">{loading ? <span className="flex justify-center items-center gap-3">Login in... <LuLoaderCircle className="animate-spin" /></span> : "Login"}</button>
                    </div>
                    <p className="text-center mt-3">Don't have an account? <Link to={"/signup"} className="text-red-500 underline">SignUp</Link></p>
                </form>
            </div>
        </div>
    );
